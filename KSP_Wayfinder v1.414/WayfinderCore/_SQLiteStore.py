...
class SQLiteJobStore:
    ...
    def upsert_job(self, params, batch_id=None, status="TODO", versions=None, result=None):
        ...
        self.conn.execute(
            ...
            INSERT INTO jobs(
                ...
                vinf_min, vinf_max, tof_encoding, opt_level, opt_injection,
    ...
    def start_run(self, job_id, versions=None, optimizer_metadata=None):
        ...
        cur = self.conn.execute(
            ...
            INSERT INTO runs(
                ...
                requested_n_island, actual_n_island, island_pop, sade_gen, n_evo_steps
            ) VALUES (?, ?, ?, ?, 'RUNNING', ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                int(job_id),
    ...
    def finish_run(self, run_id, runtime_seconds=None):
        self.conn.execute(
            """
            UPDATE runs
            SET status = 'DONE', ended_at = ?, runtime_seconds = ?, error = NULL
            WHERE id = ?
            """,
            (_utc_now(), runtime_seconds, int(run_id)),
        )
        self.conn.commit()
    ...
    def record_optimizer_snapshot(self, run_id, step, champion_fitness, champion_genes):
        ...
        self.conn.execute(
            ...
            INSERT INTO optimizer_snapshots(
                run_id, step, best_fitness, champion_fitness_json, champion_gene_json, created_at
            ) VALUES (?, ?, ?, ?, ?, ?)
    ...
    def upsert_result(self, job_id, objective_dv, result_t0, result_tof, ejection_vinf, gene, versions=None, run_id=None):
        job_id = int(job_id)
        versions = versions or {}
        now = _utc_now()
        if run_id is None:
        ...
        self.conn.execute(
            """
            INSERT INTO results(
                run_id, objective_dv, result_t0, result_tof, ejection_vinf
            ) VALUES (?, ?, ?, ?, ?)
            """,
            (
                run_id,
