...
def run_smoke_test():
    ...
    try:
        ...
        snapshot_count = store.count_rows("optimizer_snapshots")
        assert store.count_rows("optimizer_population_points") >= 3
        ...
        assert len(porkchop_points) > 0
        plotted_points = plans.plot_porkchop_from_snapshots_sqlite(
            db_path,
            run_id=best[0]["run_id"],
        )
        assert len(plotted_points) > 0
        stages = store.optimizer_stages(best[0]["run_id"])
        assert snapshot_count == sum(stage["actual_evo_steps"] for stage in stages)
        island_counts = [stage["n_island"] for stage in stages]
        assert len(island_counts) == 3
        assert all(later <= earlier for earlier, later in zip(island_counts, island_counts[1:]))
        if island_counts[0] > 1:
            assert island_counts[-1] < island_counts[0]
        assert stages[-1]["ejection_model"] == "vector_3d"
        assert stages[-1]["initialization"] == "random_plus_champion"
        print(best[0])
    finally:
        store.close()
