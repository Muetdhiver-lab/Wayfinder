...
def _planet(when, elements, mu_central_body, mu_self, radius, safe_radius, name):
	"""Create a pykep3 planet from KSP mean-anomaly elements."""
	return pk.planet(
		pk.udpla.keplerian(
			when=when,
			elem=elements,
			mu_central_body=mu_central_body,
			added_params=[mu_self, radius, safe_radius],
			name=name,
			el_type=pk.el_type.KEP_M,
		)
	)
...
EDY_TO_KDY = 4
DEFAULT_DATASTORE_NAME = "WayFinder_Vanilla"
BODY_ABBREVIATIONS = {
	"Moho": "Mo",
	"Eve": "E",
	"Kerbin": "K",
